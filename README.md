...
### 路线 A-替代：腾讯云 CloudBase 云托管（无外币卡，推荐，数据持久）
如果你没外币信用卡，用**腾讯云 CloudBase 云托管**最现实：已实名认证、给 HTTPS 域名、网络在国内。
**本项目已内置云数据库支持**——部署后数据存在 CloudBase 自带的 NoSQL 数据库里，**重启 / 重新部署都不丢**，且**不需要**对象存储（cosfs）挂载（那种挂载对频繁改写的 JSON 文件既慢又容易失败）。
...
华为云、阿里云步骤类似：找「容器服务 / 函数计算 FC / 云托管」，上传代码或连 GitHub，启动命令 `node server.js`，端口 `3000`。

---

#### 数据存储说明（重要）
程序通过 `storage.js` 支持两种存储后端，**对上层的接口完全一致**：
...
- 云数据库依赖 `@cloudbase/node-sdk`（已加入 `package.json`），由 Docker 构建时 `npm install` 安装；本地文件模式不会加载该依赖，互不影响。
