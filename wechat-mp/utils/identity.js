// 欲言信箱 —— 匿名身份生成（小程序版，使用 wx 存储）
//
// 规则：同一个"留言区"（即同一个问题）内，同一位留言者保持同一个随机昵称 /
// 头像 / 颜色；换一个问题则重新随机。昵称库含两类：①清新风格（前缀+后缀）；
// ②动植物趣味昵称（前缀短语+动物/植物名词，如"爱吃竹子的小熊猫"）。
//
// 同时复用本模块管理「删除令牌」「所有权」「已读」等本地存储，供本机删除与
// 消息通知聚合使用（替代网页版的 localStorage）。
...
function getVisitor(qid) {
  try { return wx.getStorageSync(VISITOR_KEY + qid) || null; } catch (e) { return null; }
}
function setVisitor(qid, v) {
  try { wx.setStorageSync(VISITOR_KEY + qid, v); } catch (e) {}
}
...
function getTokens() { try { return wx.getStorageSync(TOKEN_KEY) || {}; } catch (e) { return {}; } }
function getToken(id) { return getTokens()[id] || ''; }
function setToken(id, t) {
  const x = getTokens(); x[id] = t;
  try { wx.setStorageSync(TOKEN_KEY, x); } catch (e) {}
}
function getOwned() { try { return wx.getStorageSync(OWNED_KEY) || {}; } catch (e) { return {}; } }
function setOwned(id, info) {
  const x = getOwned(); x[id] = info;
  try { wx.setStorageSync(OWNED_KEY, x); } catch (e) {}
}
function getSeen() { try { return wx.getStorageSync(SEEN_KEY) || []; } catch (e) { return []; } }
function markSeen(ids) {
  const s = new Set(getSeen());
  ids.forEach((i) => s.add(i));
  try { wx.setStorageSync(SEEN_KEY, [...s]); } catch (e) {}
}

module.exports = {
  genIdentity, getVisitor, setVisitor, getOrCreateVisitor,
  deterministicAuthor, authorOf,
  getToken, setToken, getOwned, setOwned, getSeen, markSeen,
};
